...
public class PuppeteerManager(ILogger<PuppeteerManager> logger)
{
    ...
    public async Task LaunchBrowser(bool headless)
    {
        try
        {
            _browser = await Puppeteer.LaunchAsync(new LaunchOptions { Headless = headless, Args = new[] { "--disable-features=site-per-process" } }, LoggerFactory.Create(builder => builder.AddConsole()));
        }
        catch (Exception ex)
        {
